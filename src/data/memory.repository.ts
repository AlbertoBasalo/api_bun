 ...
 */
export async function selectAll(collection: string): Promise<Option<unknown[]>> {
 ...
 */
export async function selectByKeyValue(collection: string, key: string, value: string): Promise<Option<unknown[]>> {
 ...
 */
export async function selectById(collection: string, id: string): Promise<Option<unknown>> {
  const selectedData = await selectByKeyValue(collection, "id", id);
  if (selectedData.result && selectedData.result.length > 0) {
    return { result: selectedData.result[0] };
  }
  return {};
}
 ...
 */
export async function selectByContent(collection: string, content: string): Promise<Option<unknown[]>> {
  const selectedData = await selectAll(collection);
  let result: unknown[] = [];
  if (selectedData.result) {
...
// ToDo: throw error if item already exists on insert or not found on update
 ...
 */
export async function insert(collection: string, item: any): Promise<Option<unknown>> {
 ...
 */
export async function update(collection: string, id: string, item: any): Promise<Option<unknown>> {
  const collectionData = await readCollection(collection);
 ...
 */
export async function deleteById(collection: string, id: string): Promise<Option<unknown>> {
  const collectionData = await readCollection(collection);
  const index = collectionData.findIndex((i: any) => i.id == id);
  if (index === -1) {
    return { error: `Item ${id} not found on ${collection}` };
